...
pub fn build_clap_app() -> clap::Command {
    ...
    let session_options = clap::Command::new("Zingo CLI").version(version::VERSION)
            ...
            .arg(Arg::new("chain")
                ...
                .value_name("CHAIN")
                .help(
                    r#"What chain to expect. One of "mainnet", "testnet", "swarm-testnet", or "regtest". Defaults to "mainnet""#
                ))
            .arg(Arg::new("seed")
                .short('s')
                .long("seed")
...
If you don't remember the block height, you can pass '--birthday 0' to scan from the start of the blockchain."
    ...
    #[cfg(feature = "clearnet-test-mode")]
    #[error("provide --server for SWARM testnet")]
        ...
        let (server, ranked_servers) = match communications {
            ...
            Communications::Online => {
                if matches
                    ...
                    .is_some_and(|chain| chain == zingolib::config::SWARM_TESTNET_NAME)
                    && matches.get_one::<http::Uri>("server").is_none()
                {
                    return Err(ConfigTemplateError::SwarmIndexerRequired);
                }
